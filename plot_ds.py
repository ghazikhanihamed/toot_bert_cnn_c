...
membrane_train = ionchannel_membrane_train_df[ionchannel_membrane_train_df["label"]
                                              == settings.MEMBRANE_PROTEINS].shape[0]
...
ax.set_xticklabels(('Ion channels', 'Ion transporters', 'Membrane proteins'))
...
plt.savefig(settings.PLOT_PATH + "ionchannels_iontransporters_membrane_imbalanced.png",
            dpi=300, bbox_inches='tight')
plt.close()

# Read the csv files for the balanced datasets
ionchannel_membrane_train_df = pd.read_csv(
    settings.DATASET_PATH + settings.IONCHANNELS_MEMBRANEPROTEINS + "_" + "balanced" + "_" + settings.TRAIN + ".csv")
ionchannel_membrane_test_df = pd.read_csv(
    settings.DATASET_PATH + settings.IONCHANNELS_MEMBRANEPROTEINS + "_" + "balanced" + "_" + settings.TEST + ".csv")
iontransporter_membrane_train_df = pd.read_csv(
    settings.DATASET_PATH + settings.IONTRANSPORTERS_MEMBRANEPROTEINS + "_" + "balanced" + "_" + settings.TRAIN + ".csv")
iontransporter_membrane_test_df = pd.read_csv(
    settings.DATASET_PATH + settings.IONTRANSPORTERS_MEMBRANEPROTEINS + "_" + "balanced" + "_" + settings.TEST + ".csv")
...
plt.savefig(settings.PLOT_PATH + "ionchannels_iontransporters_membrane_balanced.png",
            dpi=300, bbox_inches='tight')
plt.close()
